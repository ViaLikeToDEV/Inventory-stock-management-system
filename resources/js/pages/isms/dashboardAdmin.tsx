    ...
    <div className="flex h-screen bg-[#f4f6fb] font-sans">
      ...
      <aside className="w-64 bg-[#1e2e40] text-white flex flex-col shadow-lg z-10">
        ...
        <nav className="flex-1 pt-6">
          <ul className="space-y-2 px-2">
            ...
            {/* 🟢 2. เพิ่มปุ่ม Product ลงใน Sidebar ตรงนี้ */}
            <li>
              <button
                onClick={() => setActiveMenu('product')}
                className={`w-full flex items-center px-6 py-3 rounded-lg mx-2 transition-colors ${
                  activeMenu === 'product' ? 'bg-[#2b3e52] text-white border-l-4 border-blue-400' : 'text-gray-300 hover:bg-[#2b3e52]'
                }`}
              >
            ...
            <li>
              <button
                onClick={() => setActiveMenu('Preparation')}
                className={`w-full flex items-center px-6 py-3 rounded-lg mx-2 transition-colors ${
                  activeMenu === 'Preparation' ? 'bg-[#2b3e52] text-white border-l-4 border-blue-400' : 'text-gray-300 hover:bg-[#2b3e52]'
                }`}
              >
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {/* 🟢 ถ้า activeMenu เป็น 'home' ให้โชว์หน้า Dashboard และ อัปโหลด CSV */}
          {activeMenu === 'home' && (
          ...
          {/* 🟢 3. เพิ่มบล็อกสำหรับแสดงหน้า Product */}
          {activeMenu === 'product' && (
            <div className="p-8 bg-[#eef1f8] min-h-full">
              <h2 className="text-3xl font-bold text-gray-800 mb-8">Product Management</h2>

              {/* เรียกใช้ Component Product ที่ Import มา */}
              <Product />
          ...
          {/* (ซ่อนไว้ เผื่อจะใช้หน้า packing ธรรมดา) */}
          {activeMenu === 'packing' && (
            <div className="p-8 bg-[#eef1f8] min-h-full">
